 ...
 * @version    $Id: class.ts3viewer.php 40 2011-10-06 08:42:27Z bRunO $
 */
class TS3Viewer {
    ...
    private function player_icon($var) {
        foreach ($this->icons as $key => $opt) {
            if (isset($var[$key]) and $var[$key] == $opt and file_exists($this->img_path . $key . '.png')) {
                return $key . '.png';
            }
        }
    ...
    public function tree_head() {
        if (!empty($this->sinfo['virtualserver_name'])) {
            return '<span class="ts3head"><img src="' . $this->img_path . 'serverimg.png" alt="" title="' . $this->rep($this->sinfo['virtualserver_welcomemessage']) . '"/>&nbsp;<a href="ts3server://'.$this->ts3_host.":".$this->ts3_port.'">' . $this->rep($this->sinfo['virtualserver_name']) . "</a></span>\n";
        }
        else {
            return false;
        }
    ...
    public function tree($channel=0, $wildcard='') {
        ...
        foreach ($this->clist as $key => $var) {
            ...
            if (preg_match("#\[(.*)spacer.*\](.*)#", htmlspecialchars($this->rep($var['channel_name'])), $spacer) && !$var['pid']) {
